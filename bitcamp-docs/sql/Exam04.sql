...
이런 문제가 발생한 이유?
- 다른 테이블의 데이터를 참조하는 경우, 참조 데이터의 존재 유무를 검사하는 제약을 걸지 않았기 떄문이다.
- 테이블의 데이터를 삭제할 떄 다른 테이블의 참조하는지 여부를 검사하지 않기 때문이다.

해결책?
- 다른 테이터를 참조하는 경우 해당 데이터의 존재 유무를 검사하도록 강제해야 한다.
- 다른 데이터에 의해 참조되는지 여부를 검사하도록 강제한다.
- 이것을 가능하게 하는 문법이 "외부키(FK(Foreign Key)" 이다.
