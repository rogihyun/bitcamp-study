...
//      javax.servlet.Registration.Dynamic registration) {
//    // Servlet 3.0 API의 파일 업로드를 사용하려면 다음과 같이
//    // DispatcherServlet에 설정을 추가해야 한다.
//    // 즉 멀티파트의 데이터를 PArt 객체로 받을떄는
//    // 이 설정을 추가해야 한다.
//    //
//    // 만약 Spring의 방식으로 파일 업로드를 처리하고 싶다면,
//    // AppConfig.java의 MultipartResolver를 확인하라!
//    // => 즉 멀티 파트 데이터를 MultipartFile 객체로 받을 떄는
//    // 이설정으로 처리할 수 없다.
//    // =>Appcomfing에 별도로 mulitpartResolver를 추가해야 한다.
//    // => 단 이설정을 추가하면 Spring WebMVC의 MultipartResolver가 작동되지않는다.
//    //
