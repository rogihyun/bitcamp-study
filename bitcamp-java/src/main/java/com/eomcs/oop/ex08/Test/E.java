...
public class E extends Object {
  ...
  E(String s){
    // super 클래스의 생성자나 자신의 생성자나 모두 첫 문자응로 와야하기 떄문에
    // 다음과 같이 나란히 호출할 수 없다.
    //super();
    //this();
  }
  ...
  E(int a, int b){
    //수퍼 클래스이 생성자 호출을 생략하면 기본으로 수퍼 클래스의 default 생성자를 호출한다.
    // super();
  }
...
class TestE {
  ...
  E3 obj;
  obj = new E3();
  //생성자 호출은 E3() 생성자부터 상위 클래스로 따라 올라가면서호출한다.
  // 결국 실행은 상위 클래스의 생정자부터 실행하면서 내려온다.
